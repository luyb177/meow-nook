	...
	for {
		...
		default:
			...
			if env.NextRunAt > now {
				// 重新入队 retry（保持 NextRunAt 不变），避免队头阻塞
				env.NextRunAt += int64(rand.Intn(2)) // +0~1s，避免大量消息同时到期导致的突发流量
				if err = m.producer.writeEnvelope(m.ctx, m.producer.cfg.Topics.Retry, &env); err != nil {
					logger.Error("requeue retry message error", zap.Error(err))
					// 不 commit，避免丢
					continue
				}
				_ = m.reader.CommitMessages(m.ctx, msg)
