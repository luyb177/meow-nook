package logic

import (
	"context"

	"github.com/luyb177/meow-nook/common/errorx"
	"github.com/luyb177/meow-nook/service/user/internal/svc"
	v1 "github.com/luyb177/meow-nook/service/user/pb/user/v1"

	"github.com/zeromicro/go-zero/core/logx"
)

type GetCreditPointsLogic struct {
	ctx    context.Context
	svcCtx *svc.ServiceContext
	logx.Logger
}

func NewGetCreditPointsLogic(ctx context.Context, svcCtx *svc.ServiceContext) *GetCreditPointsLogic {
	return &GetCreditPointsLogic{
		ctx:    ctx,
		svcCtx: svcCtx,
		Logger: logx.WithContext(ctx),
	}
}

// Credit
func (l *GetCreditPointsLogic) GetCreditPoints(in *v1.GetCreditPointsReq) (*v1.GetCreditPointsResp, error) {
	if in.UserId <= 0 {
		return nil, errorx.Wrap(errorx.CodeBadRequest, "user_id 不能为空", errorx.ErrBadRequest)
	}

	u, err := l.svcCtx.Repo.User.FindByID(l.ctx, in.UserId)
	if err != nil {
		return nil, errorx.WrapInternal("查询用户失败", err)
	}
	if u == nil {
		return nil, errorx.ErrUserNotFound
	}

	return &v1.GetCreditPointsResp{CreditPoints: u.Points}, nil
}
