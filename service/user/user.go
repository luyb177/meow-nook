...
var configFile = flag.String("f", "etc/user.yaml", "config file path")
...
func main() {
	...
	s := zrpc.MustNewServer(c.RpcServerConf, func(grpcServer *grpc.Server) {
		server.RegisterUserServer(grpcServer, ctx)
		reflection.Register(grpcServer)
	})
	defer s.Stop()

	logger.Info("user rpc service starting on " + c.ListenOn)
	s.Start()
}
